...
class Tiles:
    ...
    def index_window(self) -> Tuple:
        name_to_numbers = dict()
        numbers_to_name = dict()

        # Indexing Vimspector window
        for w in vim.windows:
            buf_name = str(vim.eval(f"bufname(winbufnr({w.number}))"))

            if "Variables" in buf_name:
                name_to_numbers = self._append_dict(
                    name_to_numbers, WINDOW_VARIABLE, w.number
                )
                numbers_to_name[w.number] = WINDOW_VARIABLE
            elif "StackTrace" in buf_name:
                name_to_numbers = self._append_dict(
                    name_to_numbers, WINDOW_STACK, w.number
                )
                numbers_to_name[w.number] = WINDOW_STACK
            elif "Console" in buf_name:
                name_to_numbers = self._append_dict(
                    name_to_numbers, WINDOW_CONSOLE, w.number
                )
                numbers_to_name[w.number] = WINDOW_CONSOLE
            elif "vimspector.Output" in buf_name:  # Updated this condition
                name_to_numbers = self._append_dict(
                    name_to_numbers, WINDOW_OUTPUT, w.number
                )
                numbers_to_name[w.number] = WINDOW_OUTPUT
            elif "term://" in buf_name:  # Updated terminal detection
                name_to_numbers = self._append_dict(
                    name_to_numbers, WINDOW_SHELL, w.number
                )
        ...
        for w in vim.windows:
            if w.number > stack_number and w.number < output_number:
                name_to_numbers = self._append_dict(
                    name_to_numbers, WINDOW_CODE, w.number
                )
        ...
        if number_of_lines < 70:
            ...
            for w in vim.windows:
                name = numbers_to_name.get(
                    w.number, WINDOW_UNKNOWN + "_" + str(w.buffer)
                )
